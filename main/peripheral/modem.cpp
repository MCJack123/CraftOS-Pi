...
static uint16_t open_channels[128] = {0};
...
static void check_size(uint8_t** start, uint8_t** end, uint8_t** ptr, size_t cap) {
    if (*ptr + cap >= *end) {
        uint8_t* newptr = (uint8_t*)realloc(*start, *end - *start + cap + 1024);
...
static inline float get_distance(int8_t tx_power, int8_t rx_power, uint8_t channel) {
    float mwtx = powf(10, tx_power / 40.0f);
    float mwrx = powf(10, rx_power / 10.0f);
    float db = 10.0f * log10f(mwrx / mwtx);
    float freq = 2407.0f + channel * 5.0f;
    return powf(10, (27.55f - 20.0f * log10f(freq) - db) / 20.0f);
}

static void modem_message_event(lua_State *L, void* arg) {
    struct modem_message_buf* buf = (struct modem_message_buf*)arg;
    uint8_t* ptr = buf->data;
    decode_value(L, &ptr);
    free(buf);
}

static void modem_cb(void* buf) {
    struct modem_80211_pkt* pkt = (struct modem_80211_pkt*)buf;
    ...
    if (pkt->total_frags == 1) {
        ...
        struct modem_message_buf* buf = (struct modem_message_buf*)malloc(sizeof(struct modem_message_buf) + pkt->size);
        buf->size = pkt->size;
        ...
        event.modem.distance = 0; //get_distance(pkt->tx_power, pkt_buf->rx_ctrl.rssi, pkt_buf->rx_ctrl.channel);
    ...
    list = (struct modem_message_buf*)malloc(sizeof(struct modem_message_buf) + pkt->total_size);
    list->id = pkt->id;
...
static int modem_transmit(lua_State *L) {
    static uint8_t modem_buf[1450];
    static struct modem_80211_pkt* pkt = (struct modem_80211_pkt*)modem_buf;
    uint16_t channel = luaL_checkinteger(L, 1);
    uint16_t replyChannel = luaL_checkinteger(L, 2);
    luaL_checkany(L, 3);
    lua_settop(L, 3);
    uint8_t* enc = (uint8_t*)malloc(1024);
